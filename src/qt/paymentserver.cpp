...
const QString BITCOIN_IPC_PREFIX("huadiecoin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click huadiecoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start huadiecoin: click-to-pay handler");
    else
...
{
    // clicking on huadiecoin: URLs creates FileOpen events on the Mac:
